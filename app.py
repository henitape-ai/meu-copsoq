...
if submit:
    ...
    try:
        ...
        st.success(f"✅ Avaliação de {funcao} registrada!")
        st.balloons()
    except Exception as e:
        st.error(f"Erro na gravação: {e}")

# 5. ÁREA RESTRITA (Gráfico Corrigido)
if st.session_state.df_grafico is not None:
    st.markdown("---")
    with st.expander("🔐 Visualizar Análise Técnica"):
        senha = st.text_input("Senha:", type="password", key="auth_final")
        if senha == "1234":
            st.subheader(f"📊 Laudo: {funcao} - {empresa}")
            
            # REMOVIDO 'markers=True' para evitar o erro da imagem
            ...
            resumo = "🚩 ALTO RISCO" if d['Exigências'] > 75 or d['Saúde'] > 75 else "✅ NORMAL"

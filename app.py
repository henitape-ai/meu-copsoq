...
st.title("Avaliação Psicossocial - (COPSOQ - Copenhagen Psychosocial Questionnaire)")
...
with tab1:
    ...
    with st.form("form_coleta_v13"):
        ...
        with col2:
            ...
            p9 = st.radio("TEM RECEIO DE SER DEMITIDO EM BREVE?", list(escala.keys()), index=None)
        ...
        try:
            ...
            if not df.empty:
                ...
                with c_f1:
                    ...
                    df_emp = df[df['Empresa'] == emp_sel] if emp_sel else pd.DataFrame()
                with c_f2:
                    if not df_emp.empty:
                        ...
                        df_set = df_emp[df_emp['Setor'] == set_sel] if set_sel else pd.DataFrame()
                    else:
                        st.info("Selecione uma empresa para listar os setores.")
                        df_set = pd.DataFrame()
